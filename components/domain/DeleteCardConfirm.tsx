    ...
    <Modal>
      <ModalTrigger asChild>
        <button
          ...
          className="inline-flex h-12 w-10 shrink-0 items-center justify-center rounded-sm border-[1.5px] border-danger bg-bg-base text-danger"
        >
          <Trash2 size={16} aria-hidden />
        </button>
      </ModalTrigger>
      <ModalContent title={strings.title}>
        <p className="text-[17px] text-text-secondary">{strings.body}</p>
        <form action={action} className="mt-5 flex gap-3">
          ...
          <button
            type="submit"
            className="inline-flex h-12 flex-1 items-center justify-center rounded-md bg-danger text-[17px] font-bold text-white"
          >
            {strings.confirm}
          </button>
        </form>
