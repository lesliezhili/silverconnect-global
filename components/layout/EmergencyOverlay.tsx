...
import { AlertTriangle, Phone as PhoneIcon, X, MapPin } from "lucide-react";
...
export function EmergencyOverlay({ country = "AU" }: { country?: CountryCode }) {
  ...
  if (!open) {
    ...
    return (
      <button
        ...
        className={cn(
          "fixed bottom-[180px] right-5 z-30 flex h-14 w-14 items-center justify-center rounded-full bg-danger text-white shadow-card-hover",
          "sm:bottom-[88px]"
        )}
      >
        <span aria-hidden className="text-[17px] font-extrabold leading-none">
          SOS
        </span>
      </button>
    );
  ...
  const subtitleKey = `subtitle${country}` as SubtitleKey;
  ...
  return (
    ...
    >
      ...
      {/* Secondary: Notify emergency contact */}
      <button
        type="button"
        className="h-14 w-full max-w-[320px] rounded-md border-[1.5px] border-white/30 bg-white/10 text-[17px] font-semibold text-white"
      ...
      {/* GPS indicator */}
      <div className="flex items-center gap-2 text-[15px] text-[#4ADE80]">
        <MapPin size={16} aria-hidden />
        <span>{t("gpsShared")}</span>
      </div>

      <p className="text-[16px] text-[#94A3B8]">{t("close")}</p>
    </div>
  );
}
