    ...
    return (
      <AdminShell email={admin.email ?? ""}>
        ...
        <div className="mt-2 flex flex-wrap items-center justify-between gap-2">
          ...
          <span
            className={
              "inline-flex h-7 items-center gap-1 rounded-sm px-2.5 text-[16px] font-bold uppercase " +
        ...
        <section className="mt-5 grid grid-cols-1 gap-3 md:grid-cols-2">
          <div className="rounded-lg border border-border bg-bg-base p-4">
            <p className="text-[16px] text-text-tertiary">{t("colReporter")}</p>
            <p className="mt-0.5 text-[17px] font-bold">{reporterName}</p>
            {reporter && (
              <p className="text-[16px] text-text-tertiary">{reporter.email}</p>
            )}
          </div>
          <div className="rounded-lg border border-border bg-bg-base p-4">
            ...
            {row.bookingId ? (
              ...
              <p className="mt-0.5 text-[17px] text-text-tertiary">—</p>
            )}
          </div>
          <div className="rounded-lg border border-border bg-bg-base p-4">
            <p className="text-[16px] text-text-tertiary">{t("colSubmitted")}</p>
          ...
          <div className="rounded-lg border border-border bg-bg-base p-4">
            <p className="text-[16px] text-text-tertiary">{t("colStatus")}</p>
            {reviewed ? (
              <p className="mt-0.5 text-[16px] font-semibold text-success">
                Reviewed
                {reviewerName ? ` · ${reviewerName}` : ""}
              </p>
            ) : (
              <p className="mt-0.5 text-[16px] font-semibold text-warning">
                Open
              </p>
            )}
          </div>
        ...
        <section className="mt-5 rounded-lg border border-border bg-bg-base p-5">
          ...
          <p className="mt-2 whitespace-pre-line rounded-md border border-border bg-bg-surface-2 p-3 text-[16px]">
            {row.body}
          </p>
        </section>
        ...
        {row.photos && row.photos.length > 0 && (
          <section className="mt-5 rounded-lg border border-border bg-bg-base p-5">
            <p className="text-[16px] font-bold">Photos</p>
