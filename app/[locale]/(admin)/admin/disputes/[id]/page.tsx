  ...
  try {
    ...
    return (
      <AdminShell email={admin.email ?? ""}>
        <Link
          href="/admin/disputes"
          className="inline-flex h-12 items-center gap-1 text-[17px] font-semibold text-brand"
        >
        ...
        <div className="mt-2 flex flex-wrap items-center justify-between gap-2">
          ...
          <span
            className={
              "inline-flex h-7 items-center rounded-sm px-2.5 text-[16px] font-bold uppercase tracking-wide " +
        ...
        {error === "invalidAmount" && (
          <div
            role="alert"
            className="mt-3 rounded-md border-[1.5px] border-danger bg-danger-soft px-3.5 py-3 text-[16px] font-semibold text-danger"
          >
        ...
        <section className="mt-5 grid grid-cols-1 gap-3 md:grid-cols-2">
          <div className="rounded-lg border border-border bg-bg-base p-4">
            <p className="text-[16px] text-text-tertiary">{t("colCustomer")}</p>
            <p className="mt-0.5 text-[17px] font-bold">{customerName}</p>
            {customer && (
              <p className="text-[16px] text-text-tertiary">{customer.email}</p>
            )}
          </div>
          <div className="rounded-lg border border-border bg-bg-base p-4">
            <p className="text-[16px] text-text-tertiary">{t("colProvider")}</p>
            <p className="mt-0.5 text-[17px] font-bold">{providerName}</p>
            {provider && (
              <p className="text-[16px] text-text-tertiary">{provider.email}</p>
            )}
          </div>
          <div className="rounded-lg border border-border bg-bg-base p-4">
            <p className="text-[16px] text-text-tertiary">Booking</p>
            <Link
              href={`/admin/bookings?id=${row.bookingId}`}
              className="mt-0.5 block text-[17px] font-bold text-brand tabular-nums"
            >
              B-{row.bookingId.slice(0, 8)}
            </Link>
            <p className="text-[16px] text-text-tertiary uppercase">
              {row.bookingStatus}
            </p>
          </div>
          <div className="rounded-lg border border-border bg-bg-base p-4">
            <p className="text-[16px] text-text-tertiary">{t("colAmount")}</p>
            <p className="mt-0.5 text-[17px] font-bold tabular-nums">
              {row.bookingCurrency} {total.toFixed(2)}
            </p>
            {row.resolutionAmount && (
              <p className="text-[16px] text-text-tertiary tabular-nums">
        ...
        <section className="mt-5 rounded-lg border border-border bg-bg-base p-5">
          <p className="text-[16px] font-bold">{t("disputeTimeline")}</p>
          <p className="mt-2 whitespace-pre-line rounded-md border border-border bg-bg-surface-2 p-3 text-[16px] text-text-primary">
            {row.reason}
          </p>
        </section>
        ...
        {messages.length > 0 && (
          <section className="mt-5 rounded-lg border border-border bg-bg-base p-5">
            <p className="text-[16px] font-bold">Conversation</p>
            <ul className="mt-2 flex flex-col gap-2">
              {messages.map((m) => (
                <li
                  key={m.id}
                  className={
                    "rounded-md border border-border p-3 text-[17px] " +
        ...
        {evidence.length > 0 && (
          <section className="mt-5 rounded-lg border border-border bg-bg-base p-5">
            <p className="text-[16px] font-bold">Evidence</p>
            <ul className="mt-2 flex flex-col gap-2 text-[17px]">
              {evidence.map((e) => (
                <li
                  key={e.id}
        ...
        {!isClosed ? (
          ...
          <section className="mt-6 rounded-lg border border-border bg-success-soft p-5">
            ...
            {row.decidedAt && (
              <p className="mt-1 text-[16px] text-text-tertiary tabular-nums">
            ...
            {row.decisionNote && (
              <p className="mt-2 whitespace-pre-line text-[16px]">
                {row.decisionNote}
              </p>
            )}
          </section>
    ...
    console.error("[[id]] DB unavailable", error);
    return (
