...
export default async function AdminDisputesPage({
  ...
  try {
      ...
      <AdminShell email={admin.email ?? ""}>
        ...
        <div className="mt-2 overflow-hidden rounded-lg border border-border bg-bg-base">
          {rows.length === 0 ? (
            ...
            <table className="w-full table-fixed text-left text-[17px]">
    ...
    console.error("[disputes] DB unavailable", error);
    return (
...
function Th({
  ...
  return (
    <th
      className={`px-4 py-3 text-[16px] font-semibold uppercase tracking-wide ${className}`}
    >
      {children}
    </th>
  );
    ...
    <>
      ...
      >
        ...
        <div className="flex-1 px-5 py-5">
          <dl className="grid grid-cols-2 gap-2 text-[17px]">
            <div>
          ...
          <p className="mt-5 text-[16px] font-bold">{t("disputeTimeline")}</p>
          ...
          {messages.length > 0 && (
            <>
              <p className="mt-5 text-[16px] font-bold">Conversation</p>
              <ul className="mt-2 flex flex-col gap-2">
                {messages.map((m) => (
                  <li
                    key={m.id}
                    className="rounded-md border border-border bg-bg-base p-3 text-[17px]"
                  >
          ...
          {row.status !== "decided" && row.status !== "closed" ? (
            ...
            <p className="mt-6 rounded-md bg-success-soft px-3.5 py-3 text-[16px] font-semibold text-success">
              Already decided.
            </p>
          )}
        </div>
