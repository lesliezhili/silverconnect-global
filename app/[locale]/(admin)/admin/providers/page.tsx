...
export default async function AdminProvidersPage({
  ...
  try {
    ...
    console.error("[providers] DB unavailable", error);
    return (
    ...
    <>
      ...
      >
        ...
        <div className="flex-1 px-5 py-5">
          ...
          {row.bio && (
            <>
              <p className="mt-5 text-[16px] font-bold">Bio</p>
          ...
          <p className="mt-5 text-[16px] font-bold">{t("providerStepCheck")}</p>
          {docs.length === 0 ? (
            <p className="mt-2 text-[17px] text-text-tertiary">
            ...
            <ul className="mt-2 flex flex-col gap-1.5">
              {docs.map((d) => (
                <li
                  key={d.type}
                  className="flex items-center justify-between rounded-md border border-border bg-bg-base px-3 py-2 text-[17px]"
                >
                  <span>{d.type}</span>
                  <span
                    className={
