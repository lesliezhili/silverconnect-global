...
const COUNTRIES = ["AU", "CN", "CA", "US", "TW", "SG", "HK", "MY"] as const;
...
export default async function AdminAnalyticsPage({
  ...
  try {
    ...
    return (
      <AdminShell email={admin.email ?? ""}>
          ...
          {ranges.map((r) => {
            ...
            return (
              <Link
                ...
                aria-selected={on}
                className={
                  "inline-flex h-9 items-center rounded-pill border-[1.5px] px-3 text-[17px] font-semibold " +
                  (on
        ...
        <section className="mt-4 rounded-lg border border-border bg-bg-base p-5">
          <p className="text-[16px] font-bold">{t("metricOrders")}</p>
          ...
          <ul className="mt-4 flex flex-wrap gap-3 text-[16px]">
            {ordersByCountry.map((c) => (
        ...
        <section className="mt-5 grid grid-cols-2 gap-3 lg:grid-cols-5">
          {kpiBlocks.map((k) => (
            ...
            >
              <p className="text-[16px] text-text-tertiary">{k.label}</p>
    ...
    console.error("[analytics] DB unavailable", error);
    return (
