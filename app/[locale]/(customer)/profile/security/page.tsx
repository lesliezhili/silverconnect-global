...
async function changePassword(formData: FormData) {
  ...
  if (next.length < 8) {
    nextRedirect(`/${locale}/profile/security?error=weak`);
    return;
  }
  ...
  if (!user || !(await verifyPassword(current, user.passwordHash))) {
    nextRedirect(`/${locale}/profile/security?error=wrong`);
    return;
  }
      ...
      >
        ...
        <section className="mt-8 flex items-start gap-4 rounded-lg border border-border bg-bg-base p-4">
          ...
          <div className="min-w-0 flex-1">
            ...
            <button
              type="button"
              className="mt-3 inline-flex h-12 items-center rounded-md border-2 border-brand bg-bg-base px-5 text-[17px] font-bold text-brand"
            >
              {t("twoFactorEnable")}
            </button>
          </div>
        ...
        <section className="mt-6">
          ...
          <ul className="mt-3 overflow-hidden rounded-lg border border-border bg-bg-base">
            <li className="flex items-center gap-3 px-4 py-3.5">
              <span
                aria-hidden
                className="flex h-12 w-10 items-center justify-center rounded-md bg-success-soft text-success"
              >
                <Smartphone size={20} />
              </span>
              <div className="min-w-0 flex-1">
                <p className="text-[17px] font-bold">{t("sessionThis")}</p>
          ...
          <form action={`/${locale}/auth/logout`} method="POST">
            <button
              type="submit"
              className="mt-3 inline-flex h-12 w-full items-center justify-center rounded-md border-[1.5px] border-danger bg-bg-base text-[17px] font-bold text-danger"
            >
              {t("signOutAll")}
            </button>
          </form>
