...
export default async function PaymentSuccessPage({
  ...
  try {
      ...
      <>
        ...
        <main id="main-content" className="mx-auto flex w-full max-w-content flex-col items-center bg-bg-surface px-5 pb-[120px] sm:pb-12 pt-2 text-center">
          ...
          <section className="mt-5 w-full rounded-lg border border-border bg-bg-base p-4 text-left">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="text-[17px] text-text-tertiary">{serviceLine}</p>
              </div>
            </div>
          ...
          <div className="mt-3.5 flex w-full flex-col gap-2.5">
            ...
            <Link
              href={`/bookings/${id}`}
              className="inline-flex h-12 items-center justify-center text-[17px] font-semibold text-brand"
            >
              {t("viewBooking")} →
            </Link>
            <Link
              href={`/bookings/${id}/review`}
              className="inline-flex h-12 items-center justify-center text-[17px] font-semibold text-success"
            >
              ⭐ Leave a Review
            </Link>
          </div>
    ...
    console.error("[success] DB unavailable", error);
    return (
