...
export default async function SearchPage({
  ...
  try {
      ...
      <>
        ...
        >
          ...
          <form method="get" className="mt-4 flex gap-2">
            ...
            <button
              type="submit"
              className="inline-flex h-touch-btn items-center gap-2 rounded-md bg-brand px-5 text-[17px] font-bold text-white"
            >
          ...
          {!q && (
            <p className="mt-6 text-center text-[16px] text-text-tertiary">
              {t("submitToSearch")}
            </p>
          )}
          ...
          {empty && (
            <p className="mt-4 rounded-lg border border-dashed border-border-strong bg-bg-base px-5 py-8 text-center text-[16px] text-text-secondary">
              {t("noResults")}
            </p>
          )}
          ...
          {providers.length > 0 && (
            <section className="mt-5">
              <h3 className="text-[16px] font-bold uppercase tracking-wide text-text-tertiary">
                {t("sectionProviders")}
              </h3>
                  ...
                  <li key={p.id}>
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-[17px] text-text-tertiary">
                          {p.categories
                            .map((c) =>
                              tCategories(
              ...
              <ul className="mt-2 flex flex-wrap gap-2">
                {services.map((c) => (
                  <li key={c.key}>
                    <Link
                      href={`/services/${c.key}`}
                      className="inline-flex h-12 items-center rounded-pill border-[1.5px] border-brand bg-brand-soft px-4 text-[16px] font-semibold text-brand"
                    >
                      {c.label}
                    </Link>
                  </li>
              ...
              <ul className="mt-2 flex flex-col gap-2">
                {articles.map((a) => (
                  <li key={a.slug}>
                    <Link
                      href={`/help/${a.slug}`}
                      className="block rounded-lg border border-border bg-bg-base p-3 text-[16px] font-semibold text-text-primary"
                    >
    ...
    console.error("[search] DB unavailable", error);
    return (
