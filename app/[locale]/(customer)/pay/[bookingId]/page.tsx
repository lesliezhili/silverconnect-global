...
async function processPayment(formData: FormData) {
  ...
  if (!booking) nextRedirect(`/${locale}/home`);

  // Get default payment method
  const [pm] = await db.select().from(paymentMethods)
    .where(and(eq(paymentMethods.userId, me.id), eq(paymentMethods.isDefault, true))).limit(1);

  // Create payment intent (simulated)
  const piId = `pi_sim_${randomUUID().replace(/-/g, "").slice(0, 24)}`;
  await db.insert(payments).values({
    bookingId, stripePaymentIntentId: piId,
    amount: String(booking.totalPrice), currency: booking.currency,
    status: "authorized" as never,
  });

  // Update booking to confirmed
  await db.update(bookings)
    .set({ status: "confirmed" as never, confirmedAt: new Date(), updatedAt: new Date() })
    .where(eq(bookings.id, bookingId));

  nextRedirect(`/${locale}/bookings/${bookingId}/success`);
}
      ...
      return (
        <>
          ...
          <main id="main-content" className="mx-auto flex w-full max-w-content flex-col items-center justify-center bg-bg-surface px-6 py-16 text-center">
            ...
            <p className="mt-1 text-[16px] text-text-tertiary">{t("redirecting")}</p>
          </main>
        </>
      );
    }
  ...
  return (
    <>
      ...
      <main id="main-content" className="mx-auto w-full max-w-content overflow-auto bg-bg-surface px-5 pb-[120px] sm:pb-12 pt-5">
        ...
        <p className="mt-1 flex items-center gap-1.5 text-[16px] text-text-tertiary">
        ...
        <div className="mt-4 flex flex-col gap-2.5">
          ...
          <div className="my-2 flex items-center gap-3">
            ...
            <span className="text-[17px] text-text-tertiary">{t("orCard")}</span>
            <span className="h-[1px] flex-1 bg-border" aria-hidden />
          </div>
        ...
        {state === "failed" && (
          ...
          >
            ...
            <div>
              ...
              <p className="mt-0.5 text-[16px] text-danger">{t("declinedHint")}</p>
            </div>
          </div>
        )}
        ...
        {state === "threeDS" && (
          ...
          >
            ...
            <p className="mt-1.5 text-[16px] text-brand">{t("threeDSHint")}</p>
          </div>
        )}
    ...
    console.error("[pay] DB unavailable", error);
    return (
