    ...
    <>
      ...
      >
        ...
        <section className="mt-8 rounded-lg border border-border bg-bg-base p-5">
          ...
          {exportUrl && (
            <p className="mt-3 rounded-md bg-success-soft px-3.5 py-2.5 text-[16px] text-success">
              <a
                href={exportUrl}
                download
          ...
          <form action={exportDataAction} className="mt-3">
            ...
            <button
              type="submit"
              className="inline-flex h-12 items-center rounded-md border-[1.5px] border-brand bg-bg-base px-5 text-[17px] font-bold text-brand"
            >
              {t("download")}
            </button>
          </form>
        ...
        <section className="mt-5 rounded-lg border-[1.5px] border-danger bg-danger-soft p-5">
          ...
          <p className="mt-1 text-[17px] text-danger">{t("deleteHint")}</p>
          <form action={deleteAccountAction} className="mt-3 flex flex-col gap-3">
            <input type="hidden" name="locale" value={locale} />
            <label className="text-[17px] font-semibold text-danger">
              {locale === "zh"
                ? '请输入大写 "DELETE" 以确认：'
              ...
              <input
                ...
                pattern="DELETE"
                autoComplete="off"
                className="mt-1 block w-full rounded-md border-[1.5px] border-danger bg-bg-base px-3 py-2 text-[17px] font-bold uppercase text-danger focus:outline-none"
              />
            </label>
            <button
              type="submit"
              className="inline-flex h-12 items-center justify-center rounded-md border-[1.5px] border-danger bg-bg-base px-5 text-[17px] font-bold text-danger"
            >
              {t("deleteAccount")}
            </button>
          </form>
