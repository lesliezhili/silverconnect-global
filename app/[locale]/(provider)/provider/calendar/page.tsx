  ...
  return (
    <>
      ...
      >
        ...
        <div className="mt-4 grid grid-cols-7 gap-1 text-center text-[16px] font-semibold text-text-tertiary">
          {dayKeys.map((k) => (
            <div key={k}>{t(k)}</div>
          ))}
        </div>
        ...
        <ul className="mt-6 flex flex-wrap gap-4 text-[17px]">
    ...
    console.error("[calendar] DB unavailable", error);
    return (
...
function cellClass(k: DayKind) {
  const base =
    "flex h-12 flex-col items-center justify-center rounded-md text-[16px] font-semibold tabular-nums";
