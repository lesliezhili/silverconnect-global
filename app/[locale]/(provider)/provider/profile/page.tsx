  ...
  return (
    <>
      ...
      <main id="main-content" className="mx-auto w-full max-w-content px-5 pb-[120px] pt-6 sm:pb-12">
        ...
        {saved && (
          <div role="status" className="mt-4 flex items-center gap-2 rounded-md bg-success-soft px-3.5 py-3 text-[17px] font-semibold text-success">
        ...
        <div className="mt-5 flex items-center gap-4 rounded-lg border border-border bg-bg-base p-4">
          ...
          <div className="min-w-0 flex-1">
            ...
            <Link href="/provider" className="mt-1 inline-flex h-9 items-center rounded-sm border-[1.5px] border-border-strong bg-bg-base px-3 text-[16px] font-semibold text-text-primary">
              {t("preview")}
            </Link>
          </div>
        </div>
        ...
        <form action={saveProfile} className="mt-5 flex flex-col gap-4">
          ...
          <div>
            ...
            />
            <p id="bio-hint" className="mt-1.5 text-[17px] text-text-tertiary">{t("bioHint")}</p>
          </div>

          <fieldset>
            <legend className="text-[17px] font-bold">{t("languages")}</legend>
            <div className="mt-2 flex flex-wrap gap-2">
              {LANGS.map((l) => (
                <label key={l} className="inline-flex h-12 cursor-pointer items-center rounded-pill border-[1.5px] border-border-strong bg-bg-base px-4 text-[17px] font-semibold text-text-primary has-[:checked]:border-brand has-[:checked]:bg-brand-soft has-[:checked]:text-brand">
