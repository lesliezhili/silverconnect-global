...
export default async function ProviderAvailabilityPage({
  ...
  try {
    providerId = await ensureProviderId(me.id);
    if (!providerId) {
    ...
    console.error("[availability] DB unavailable", error);
  }
    ...
    <>
      ...
      >
        ...
        <section className="mt-5 rounded-lg border border-border bg-bg-base p-4">
          <p className="text-[16px] font-bold">{t("availabilityApply")}</p>
            ...
            >
              ...
              <div className="grid grid-cols-3 gap-2">
                {SLOTS.map((s) => (
                  <label
                    key={s}
                    className="inline-flex h-12 cursor-pointer items-center justify-center rounded-sm border-[1.5px] border-border-strong bg-bg-base px-2 text-[16px] font-semibold text-text-primary has-[:checked]:border-brand has-[:checked]:bg-brand-soft has-[:checked]:text-brand"
                  >
                    <input
                      type="checkbox"
                      name={`avail_${d}`}
...
function TemplateForm({
  ...
  return (
    <form action={applyTemplateAction}>
      ...
      <button
        type="submit"
        className="inline-flex h-12 items-center rounded-pill border-[1.5px] border-brand bg-bg-base px-4 text-[16px] font-semibold text-brand"
      >
        {label}
      </button>
    </form>
