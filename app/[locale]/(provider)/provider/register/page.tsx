...
// AbnLookupInput import removed for build fix
...
const SLOT_KEYS = ["6am-9am", "9am-12pm", "12pm-3pm", "3pm-6pm", "6pm-9pm"] as const;
...
async function saveStep2(formData: FormData) {
  ...
  // ID details captured from form, persisted in finishWizard via session/DB
  void formData.get("idType");
  void formData.get("idNumber");
      ...
      >
        ...
        <form action={action} className="mt-6 flex flex-col gap-4">
          ...
          <div className="mt-4 flex items-center gap-3">
            <Link
              href={prevHref}
              className="inline-flex h-12 items-center rounded-md border-[1.5px] border-border-strong bg-bg-base px-5 text-[17px] font-semibold text-text-primary"
            >
              {tCommon("back")}
            </Link>
...
function Step1({ t, draft }: { t: T; draft: DraftSnapshot }) {
  return (
    <>
      ...
      <div>
        <>
        <label htmlFor="abn" className="block text-[18px] font-semibold text-gray-700 mb-1">ABN (Australian Business Number)</label>
        <input id="abn" name="abn" type="text" placeholder="e.g. 51 824 753 556" maxLength={14}
          className="w-full rounded-xl border-2 border-gray-300 p-4 text-[20px]" />
        <p className="mt-1.5 text-[17px] text-gray-500">Optional — for sole traders or businesses</p>
      </>
      </div>
      <div>
      ...
      <div>
        ...
        />
        <p id="bio-hint" className="mt-1.5 text-[17px] text-text-tertiary">
          {t("rBioHint")}
        </p>
      </div>
    </>
    ...
    <div className="flex flex-col gap-5">
      ...
      <div>
        ...
        <select
          id="idType"
          name="idType"
          className="block w-full h-[56px] rounded-md border-[1.5px] border-border-strong bg-bg-base px-3.5 text-[17px] text-text-primary focus:border-brand focus:outline-none"
          required
        >
          <option value="">Select ID type...</option>
          <option value="drivers_licence">Australian Driver Licence</option>
      ...
      <div>
        ...
        <Input
          ...
          name="idNumber"
          type="text"
          placeholder="e.g. Licence: 12345678 or Passport: PA1234567"
          required
        />
      </div>
      ...
      {/* Upload */}
      <div>
        <Label>Upload Photo of ID (front)</Label>
        <label className="mt-2 flex h-[80px] w-full cursor-pointer items-center justify-center gap-3 rounded-lg border-2 border-dashed border-border-strong bg-bg-base text-[17px] font-semibold text-text-secondary hover:border-brand hover:bg-brand-soft">
          <Upload size={22} aria-hidden />
          <span>Tap to upload photo</span>
          <input type="file" name="idPhoto" accept="image/*,application/pdf" className="sr-only" />
        </label>
      </div>
      ...
      <div className="rounded-lg border border-border bg-bg-base p-4">
        <label className="flex items-start gap-3 cursor-pointer">
          <input type="checkbox" name="policeCheck" value="yes" className="mt-1 h-6 w-6 rounded border-border-strong text-brand" />
          <div>
            <p className="text-[17px] font-bold">National Police Check</p>
            <p className="mt-0.5 text-[16px] text-text-tertiary">
              I have a current check (or will obtain one before starting)
            </p>
          </div>
        </label>
      </div>
      ...
      <div className="rounded-lg border border-border bg-bg-base p-4">
        <label className="flex items-start gap-3 cursor-pointer">
          <input type="checkbox" name="wwcc" value="yes" className="mt-1 h-6 w-6 rounded border-border-strong text-brand" />
          <div>
            <p className="text-[17px] font-bold">Working With Children Check</p>
            <p className="mt-0.5 text-[16px] text-text-tertiary">
  ...
  return (
    <>
      <div className="rounded-lg border border-border bg-bg-base p-4">
        ...
        <p className="mt-1 text-[17px] text-text-secondary">{t("serviceAreaHint")}</p>
        <div className="mt-3 flex h-36 items-center justify-center rounded-md bg-bg-surface-2 text-[17px] text-text-tertiary">
          {t("mapPlaceholder")}
        </div>
        <div className="mt-3">
      ...
      <fieldset>
        <legend className="text-[17px] font-bold">{t("serviceCategories")}</legend>
        <div className="mt-3 flex flex-wrap gap-2">
          {cats.map((c) => (
            <label
              key={c}
              className="inline-flex cursor-pointer items-center gap-1 rounded-pill border-[1.5px] border-border-strong bg-bg-base px-4 py-2 text-[16px] font-semibold text-text-primary has-[:checked]:border-brand has-[:checked]:bg-brand-soft has-[:checked]:text-brand"
            >
              <input
                type="checkbox"
                name="categories"
...
function Step4({ t, draft }: { t: T; draft: DraftSnapshot }) {
  return (
    <>
      <p className="text-[16px] text-text-secondary">{t("availabilityHint")}</p>
      <ul className="mt-2 flex flex-col gap-2">
        {DAY_LABELS.map((d) => {
          ...
          return (
            ...
            >
              <span className="text-[17px] font-bold">{t(DAY_KEYS[d])}</span>
              <div className="flex flex-wrap gap-2">
                {SLOT_KEYS.map((s) => (
                  <label
                    key={s}
                    className="inline-flex cursor-pointer items-center justify-center rounded-sm border-[1.5px] border-border-strong bg-bg-base px-2 py-2 text-[17px] font-semibold text-text-primary has-[:checked]:border-brand has-[:checked]:bg-brand-soft has-[:checked]:text-brand"
                  >
                    <input
                      type="checkbox"
                      name={`avail_${d}`}
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="rounded-lg border border-border bg-bg-base p-5">
        <h3 className="text-[20px] font-bold text-text-primary mb-1">Payment Details</h3>
        <p className="text-[16px] text-text-tertiary mb-4">
          Where should we send your earnings? Your bank details are encrypted and secure.
        </p>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div>
            ...
            <Input
              ...
              required
            />
            <p className="mt-1 text-[15px] text-text-tertiary">6 digits (with or without dash)</p>
          </div>
          <div>
      ...
      <div className="rounded-lg border border-border bg-bg-base p-4">
        <label className="flex items-start gap-3 cursor-pointer">
          <input type="checkbox" name="termsAccepted" value="yes" required className="mt-1 h-6 w-6 rounded border-border-strong text-brand" />
          <p className="text-[17px] text-text-secondary">
            I agree to the <span className="font-semibold text-brand">Terms of Service</span> and confirm my details are accurate
          </p>
        </label>
      </div>
    </div>
