...
export default async function CompliancePage({
  ...
  let docs: Array<{ type: DocType; row: any; state: "missing" | "valid" | "expiring" | "expired"; days: number }> = [];
  let anyExpiring = true;
  const now = new Date();
  const fmt = (d: Date) =>
  ...
  try {
    ...
    anyExpiring = docs.some(
      (d) => d.state === "expiring" || d.state === "expired" || d.state === "missing",
    );
  } catch (error) {
    console.error("[compliance] DB unavailable", error);
    docs = KNOWN_TYPES.map((type) => ({ type, row: null, state: "missing" as const, days: 0 }));
  }
  ...
  return (
    <>
      ...
      >
        ...
        <p className="mt-1 text-[17px] text-text-secondary">{t("sub")}</p>

        {anyExpiring && (
          <div
            role="alert"
            className="mt-4 flex items-center gap-2 rounded-md border-[1.5px] border-warning bg-warning-soft px-3.5 py-3 text-[16px] font-semibold text-warning"
          >
        ...
        {uploaded && (
          <div
            role="status"
            className="mt-3 rounded-md bg-success-soft px-3.5 py-2.5 text-[16px] font-semibold text-success"
          >
        ...
        {error && (
          <div
            role="alert"
            className="mt-3 rounded-md border-[1.5px] border-danger bg-danger-soft px-3.5 py-2.5 text-[16px] font-semibold text-danger"
          >
            {error === "tooLarge"
            ...
            return (
              ...
              >
                <div className="flex items-start gap-3">
                  <span
                    aria-hidden
                    className={`flex h-12 w-10 shrink-0 items-center justify-center rounded-md ${cls}`}
                  >
                    <FileText size={20} />
                  </span>
                  <div className="min-w-0 flex-1">
                    ...
                    {d.row?.expiresAt && (
                      <p className="mt-0.5 text-[17px] text-text-secondary tabular-nums">
                ...
                >
                  ...
                  <div className="flex flex-col gap-2 sm:items-stretch">
                    <Label htmlFor={`file-${d.type}`}>
                      <span className="inline-flex h-12 w-full cursor-pointer items-center justify-center gap-1.5 rounded-md border-[1.5px] border-border-strong bg-bg-base px-3 text-[17px] font-semibold text-text-primary">
        ...
        <p className="mt-5 rounded-md border border-border bg-bg-surface-2 px-3.5 py-3 text-[17px] text-text-tertiary">
