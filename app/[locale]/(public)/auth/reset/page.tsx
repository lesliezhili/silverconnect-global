  ...
  return (
    <AuthCard title={t("resetTitle")} subtitle={t("resetSub")}>
      {justSentNotice && !errorMsg && (
        <div
          role="status"
          className="mb-4 rounded-md border-[1.5px] border-success bg-success-soft px-3.5 py-3 text-[16px] font-semibold text-success"
        >
          {t("forgotSentHint")}
        </div>
      )}
      ...
      <form className="flex flex-col gap-4" action={resetAction}>
        ...
        <div>
          ...
          />
          <p id="password-hint" className="mt-1.5 text-[16px] text-text-secondary">
            {t("passwordHint")}
          </p>
        </div>
        <div>
      ...
      <Link
        href="/auth/login"
        className="mt-4 inline-flex h-12 w-full items-center justify-center text-[17px] font-semibold text-brand"
      >
        {tCommon("back")}
      </Link>
    </AuthCard>
