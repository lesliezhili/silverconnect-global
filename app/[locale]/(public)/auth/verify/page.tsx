    ...
    return (
      <AuthCard title={t("verifySuccess")} subtitle={t("verifySuccessHint")}>
        <div className="flex flex-col items-center gap-4 py-2 text-center">
          ...
          <Link
            href={sp.role === "provider" ? "/provider/register" : "/home"}
          ...
          >
            {sp.role === "provider" ? t("continueToProviderSetup") : t("continueToHome")} →
          </Link>
        </div>
      </AuthCard>
    );
    ...
    >
      ...
      <form action={verifyCodeAction} className="mt-4 flex flex-col gap-3">
        ...
        <div>
          ...
          />
          <p id="code-hint" className="mt-1.5 text-[17px] text-text-tertiary">
            {t("verifyCodeHint")}
          </p>
        </div>
        <Button type="submit" variant="primary" block size="md">
      ...
      <div className="mt-4 flex flex-col gap-2.5">
        <a
          href={`mailto:${email}`}
          className="inline-flex h-12 items-center justify-center rounded-md border-[1.5px] border-border-strong bg-bg-base text-[17px] font-semibold text-text-primary"
        >
          {t("openMailApp")}
        </a>
        <form action={resendAction}>
          ...
          <button
            type="submit"
            className="inline-flex h-12 w-full items-center justify-center rounded-md text-[17px] font-semibold text-brand"
          >
            {t("verifyResend")}
          </button>
        </form>
