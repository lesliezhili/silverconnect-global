...
function getClient(): PgClient {
  ...
  if (!url) {
    // During `next build` on Vercel, DATABASE_URL may not be available.
    // Return a dummy that will throw only when actually called at runtime.
    throw new Error(
      "DATABASE_URL not set — if this appears during build, ensure DATABASE_URL " +
      "is added to Vercel Environment Variables (Settings → Environment Variables)."
    );
  }
